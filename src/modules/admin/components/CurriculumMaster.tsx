  ...
  return <div className="space-y-5">
    <div className="bg-white rounded-3xl border border-stone-200 shadow-xs p-5">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div><div className="flex items-center gap-2 text-xs font-bold text-[#10246f] uppercase tracking-wider"><Globe2 className="w-4 h-4 text-blue-600" /> Curriculum Master</div><h2 className="text-2xl font-black text-stone-900 mt-1">Country → Region → Curriculum</h2><p className="text-xs text-stone-500 mt-1">Manage the global curriculum hierarchy used by Question Bank, Question Creator and bulk imports.</p></div>
        <div className="flex gap-2"><button onClick={openCreate} className="px-4 py-2 rounded-xl bg-[#10246f] hover:bg-[#0c1b54] text-white text-xs font-bold flex items-center gap-1.5 shadow-xs transition cursor-pointer"><Plus className="w-4 h-4" /> Add {section === 'countries' ? 'Country' : section === 'regions' ? 'Region' : 'Curriculum'}</button><button onClick={() => { setData(loadCurriculumMaster()); onSuccessMessage('Curriculum Master reloaded.'); }} className="px-3 py-2 rounded-xl border border-stone-200 text-xs font-bold flex items-center gap-1.5 hover:bg-stone-50 transition cursor-pointer"><RotateCcw className="w-3.5 h-3.5" /> Reload</button></div>
      </div>
    </div>
    ...
    {showForm && <div className="fixed inset-0 z-[120] bg-black/30 flex items-center justify-center p-4"><div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-5 space-y-4"><div className="flex justify-between"><div><h3 className="font-black">{editing ? 'Edit' : 'Add'} {section === 'countries' ? 'Country' : section === 'regions' ? 'State / Region' : 'Curriculum'}</h3><p className="text-[11px] text-stone-500 mt-1">Stable code is used by CSV/Excel imports.</p></div><button onClick={resetForm}>×</button></div>
      ...
      <div className="flex justify-end gap-2"><button onClick={resetForm} className="px-3 py-2 rounded-xl border border-stone-200 text-xs font-bold hover:bg-stone-50 transition cursor-pointer">Cancel</button><button onClick={save} className="px-4 py-2 rounded-xl bg-[#10246f] hover:bg-[#0c1b54] text-white text-xs font-bold flex items-center gap-1.5 shadow-xs transition cursor-pointer"><CheckCircle2 className="w-3.5 h-3.5" /> Save</button></div>
    </div></div>}
  </div>;
}
